...
def get_full_err_scores(test_result, val_result):
    ...
    labels = np_test_result[2, :].tolist()
...
def get_err_scores(test_res, val_res):
    ...
    for i in range(test_predict.shape[0]):
        test_gt1.append(np.array(test_gt[i]))
        test_predict1.append(np.array(test_predict[i]))
    #print(test_predict.shape)
    #print(test_gt.shape)
    test_predict1 = np.array(test_predict1)
    test_gt1 = np.array(test_gt1)
    n_err_mid, n_err_iqr = get_err_median_and_iqr(test_predict1, test_gt1)
...
def get_val_performance_data(total_err_scores, normal_scores, gt_labels, topk=1):
    ...
    total_topk_err_scores = []
    topk_err_score_map=[]
    #np.save("loss_channel.npy",total_err_scores)
    total_topk_err_scores = np.sum(np.take_along_axis(total_err_scores, topk_indices, axis=0), axis=0)
    #np.save("loss_avg.npy", total_err_scores)
    #print(total_topk_err_scores.shape)
    mean = normal_scores.mean()
    std = normal_scores.std()

    per_list = np.percentile(normal_scores, (25, 50, 75), interpolation='midpoint')
    q3 = per_list[2]
    iqr = per_list[2]-per_list[0]
    thresold2 = q3 + 1.5*iqr
    thresold = mean + 3*std
    #print(thresold)
    #print(thresold2)
    ...
    pred_labels[total_topk_err_scores > thresold2] = 1
